...
def f_find_peaks(smoothed_force, gradient_force, main_threshold=0.03, sub_threshold=0.015, distance=25):
    ...
    while i < len(smoothed_force):
        if smoothed_force[i] > main_threshold and gradient_force[i] >= 0:
            # Found the start of a peak region
            highest_peak = i
            ...
            # Find the end of the peak region
            end_offset = i
...
'''def f_find_peaks(smoothed_force, gradient_force, main_threshold=0.03, sub_threshold=0.015, distance=25):
    ...
    i = 0
    while i < len(smoothed_force):
        if smoothed_force[i] > sub_threshold:
            highest_peak = i
            while i < len(smoothed_force) and smoothed_force[i] > main_threshold:
                if smoothed_force[i] > smoothed_force[highest_peak]:
                    highest_peak = i
                i += 1
            ...
            end_offset = highest_peak
            while end_offset < len(smoothed_force) and (smoothed_force[end_offset] >= main_threshold or gradient_force[end_offset] <= 0):
                end_offset += 1

            peaks.append(highest_peak)
            start_rises.append(start_rise)
            end_offsets.append(end_offset - 1)

            # find sub-peaks in this range
            sub_peak_indices, _ = find_peaks(smoothed_force[start_rise:end_offset], height=sub_threshold, distance=distance, prominence=0.01)
            sub_peak_indices = [sp for sp in sub_peak_indices if sp + start_rise != highest_peak]  # exclude main peaks
            sub_peak_indices = [sp + start_rise for sp in sub_peak_indices] 
            sub_peaks.extend(sub_peak_indices)
...
'''def f_find_peaks(smoothed_force, gradient_force, threshold=0.03):
    peak_threshold = threshold + 0.02
    peaks = []
    start_rises = []
    end_offsets = []
    ...
    i = 0
    while i < len(smoothed_force):
        if smoothed_force[i] > peak_threshold:
            highest_peak = i
            while i < len(smoothed_force) and smoothed_force[i] > threshold:
                if smoothed_force[i] > smoothed_force[highest_peak]:
                    highest_peak = i
                i += 1

            start_rise = highest_peak
            while start_rise > 0 and (smoothed_force[start_rise] >= threshold or gradient_force[start_rise] >= 0):
                start_rise -= 1

            end_offset = highest_peak
            while end_offset < len(smoothed_force) and (smoothed_force[end_offset] >= threshold or gradient_force[end_offset] <= 0):
                end_offset += 1
    ...
    return peaks, start_rises, end_offsets'''
